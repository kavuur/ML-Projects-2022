...
class live_capturing:
    ...
    def live_video_capturing(self, in_path, exam_start_time, exam_end_time):
    ...
    def convert_vid_to_images(self, in_path, out_path):
...
def ml_predict(out_path, model):
    ...
    if len(fls) > 0:
        ...
        for file in fls:
            ...
            test_result = np.round(saved_model.predict(img11))
            print(test_result)
            image = {}
            if test_result[0][0] == 1:
                print("Non Cheating Instance")
                image ={"url":file, "status":"nc"}
            else:
                final_result.append(test_result)
                print("Cheating Instance")
                image = {"url": file, "status": "c"}
            result_arr.append(image)
